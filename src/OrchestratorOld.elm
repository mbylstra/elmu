...
updateNode : uiModel -> DictGraph idType uiModel -> AudioNode idType uiModel
...
updateNode uiModel graph node =
  case node of
    Oscillator props ->
      let
        inputs = props.inputs
        (frequencyValue, graph2) = getInputValue uiModel graph props.inputs.frequency
        (frequencyOffsetValue, graph3) = getInputValue uiModel graph props.inputs.frequencyOffset
        (phaseOffsetValue, graph4) = getInputValue uiModel graph props.inputs.phaseOffsetValue
        (newValue, newPhase) = props.func frequencyValue frequencyOffsetValue phaseOffsetValue props.state.phase
        newState = {outputValue = newValue, phase = newPhase}
        newNode = Oscillator { props | state = newState }
      in
        (newValue, replaceGraphNode newNode graph4)
